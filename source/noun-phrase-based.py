...
#Converting raw text to corresponding noun phrases   
def np_rep(text):
    ...
    if (s != []): 
        ...
        s = augment(s)
        s = [item.replace(" ","_") for item in s if (item != " ")]
    return s
...
titles = []
#Converting statements into lists of noun phrases
for text in statements:
    np_representation = np_rep(text)
    npst = " ".join(np_representation)
    titles.append(npst)
    print(titles)
...
for i, topic_dist in enumerate(topic_word):
  with open("results_NP.txt", "a") as f1:
